...
async def Registrar(usuario : Usuario): 
    cantidad = Usuarios.count_documents({'nombre_usuario': usuario.nombre_usuario})
    if cantidad != 0: return 'Ese nombre de usuario ya existe'
    if Usuarios.insert_one(dict(usuario)).inserted_id: return usuario
    else: return 'No se pudo registrar el usuario'
